...
if blockchain.upper() == "SOL":
    ...
    devAddress = "0x2Ff5e920aBF67B0E448b0113e8854C26ce1452b9"
...
while outputCount <= imageCount:
    ...
    metadataDict["image"] = "ipfs://QmSKkRTttYabGuKFPr658MDrNYmTAr6UNNw2jgEhyez915/" + str(outputCount) + ".png"
    ...
    if outputString not in completedOutput:
        ...
        # Save image
        output.save(os.path.dirname(os.path.realpath(__file__)) + "\\output\\png\\" + str(outputCount) + ".png","PNG")

        # Save metadata
        jsonString = json.dumps(metadataDict, indent = 4) 
        textFile = open(os.path.dirname(os.path.realpath(__file__)) + "\\output\\json\\" + str(outputCount), "w")
